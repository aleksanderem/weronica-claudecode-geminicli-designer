# README for AI Agent: Claude-Figma MCP Setup Guide
...
Install and configure Claude-Figma MCP integration with **AI-agent-friendly decision trees** and fail-safe checkpoints at each step. This guide uses Claude Code's built-in Git clone functionality to directly clone the official repository, simplifying the setup process.
...
# 6. Claude Code IDE Availability
CLAUDE_AVAILABLE=$(which claude-code >/dev/null 2>&1 && echo "YES" || echo "NO")
echo "CLAUDE_AVAILABLE: $CLAUDE_AVAILABLE"
...
- ELIF `CLAUDE_AVAILABLE="YES"` AND `SUDO_AVAILABLE="NO"` → **Method B: Claude-Only Setup**
...
### 📱 Method B: Claude-Only Setup
**Conditions**: `CLAUDE_AVAILABLE="YES"` AND `SUDO_AVAILABLE="NO"`
...
which claude-code && echo "✅ Claude Code available" || echo "❌ Install Claude Code IDE manually"
```
...
**🎯 Project is already cloned and open in Claude Code IDE**

1. **Verify project structure**:

```bash
# Verify current directory (should already be in claude-talk-to-figma-mcp)
pwd | grep "claude-talk-to-figma-mcp" && echo "✅ Correct directory" || echo "❌ ERROR: Wrong directory"
...
test -f .claude/mcp.json && echo "✅ MCP config created" || echo "❌ ERROR: MCP config missing"
```
...
```
CHECKPOINT 1: STARTING METHOD B - CLAUDE-ONLY SETUP
...
```bash
# Verify Claude-only setup conditions
[ "$CLAUDE_AVAILABLE" = "YES" ] && echo "✅ Claude Code IDE available" || echo "❌ ERROR: Install Claude Code IDE first"
[ "$SUDO_AVAILABLE" = "NO" ] && echo "✅ Restricted environment confirmed" || echo "⚠️ Admin access available, consider Method A"
```

### Step B.1: Verify Project is Open in Claude Code

**🎯 Project is already cloned and open in Claude Code IDE**

1. **Verify project is properly loaded**:
   - Confirm Claude Code IDE has the project open
   - Open terminal in Claude Code: `Terminal > New Terminal` (or `Ctrl+Shift+`)

### Step B.2: Setup Dependencies in Claude Code
...
### Step B.3: Follow README Setup in Claude Code

**📖 CRITICAL FOR AI AGENTS**: Ask the AI assistant in Claude Code to read and execute the README:
...
**🚨 CRITICAL FOR AI AGENTS**: Project is already cloned and open in Claude Code.
...
**CHECKPOINT C.1**: Verify in Claude Code:
...
1.  **Start the Server**: Open a new terminal in Claude Code and run the following command. The terminal will become unresponsive, which means the server is running correctly.
    ```bash
...
**🚨 CRITICAL FOR AI AGENTS**: Use Claude Code's `@Terminals` to monitor WebSocket server status:

1. **Check Active Terminals**:
   - In Claude Code, type `@Terminals` in chat
...
3. **Monitor Server Logs**:
   - Keep the WebSocket terminal visible in Claude Code
...
**🔍 IMPORTANT FOR AI AGENTS**: Use Claude Code's `@Terminals` to verify the plugin connection:
...
if [ "$CLAUDE_AVAILABLE" = "NO" ]; then
    echo "❌ CRITICAL: Install Claude Code IDE first - https://claude.ai/code"
    exit 1
...
if [ ! -f "package.json" ] || [ ! -d "src" ]; then
    ...
    echo "💡 Please ensure you're in the cloned claude-talk-to-figma-mcp directory"
    echo "💡 Use Claude Code's File > Open Folder to open the cloned project"
    exit 1
fi
...
echo "💡 Restarting Claude Code to reload MCP configuration..."
claude-code .
sleep 5
```
...
# Required Tools  
[ "$CLAUDE_AVAILABLE" = "YES" ] && echo "✅ Claude Code IDE Available" || echo "❌ Claude Code Missing"
...
# Project Setup
[ -f ".claude/mcp.json" ] && echo "✅ MCP Config Present" || echo "❌ MCP Config Missing"
...
**Execute in Claude Code with MCP connected:**
...
- ✅ Claude Code IDE launched from project directory
...
- ✅ `join_channel` command works in Claude Code
...
### Recommended Setup (Use Claude Code's Built-in Git Clone)
...
5. **Open cloned project in Claude Code**
6. **Run setup**: `bun setup`

### Alternative One-Line Commands (For Advanced Users)
```bash
# Method A1 (Homebrew) - Clone and auto-open in Claude Code
[ "$HOMEBREW_AVAILABLE" = "YES" ] && cd ~/Desktop && git clone https://github.com/sonnylazuardi/claude-talk-to-figma-mcp.git && cd claude-talk-to-figma-mcp && brew install bun && bun setup && claude-code .

# Method A2 (Direct) - Clone and auto-open in Claude Code
[ "$SUDO_AVAILABLE" = "YES" ] && cd ~/Desktop && git clone https://github.com/sonnylazuardi/claude-talk-to-figma-mcp.git && cd claude-talk-to-figma-mcp && curl -fsSL https://bun.sh/install | bash && source ~/.zshrc && bun setup && claude-code .

# Method B (Claude-only) - Clone and open manually
[ "$CLAUDE_AVAILABLE" = "YES" ] && cd ~/Desktop && git clone https://github.com/sonnylazuardi/claude-talk-to-figma-mcp.git && echo "✅ Project cloned to ~/Desktop/claude-talk-to-figma-mcp" && echo "💡 Open this folder in Claude Code and run 'bun setup'"
```
...
**🔍 RECOMMENDED FOR AI AGENTS**: Use Claude Code's `@Terminals` for real-time monitoring:

1. **Check Active Services**:
   - Type `@Terminals` in Claude Code chat anytime
...
**Remember**: Always keep the WebSocket server running for the Figma plugin to communicate with Claude Code! Use `@Terminals` to monitor its health. 